...
All notable changes to StructFrame will be documented in this file.
...
- **Registry system**: `register_backend()` allows third-party backends to be registered without modifying StructFrame source
...
- **LazyFrame transparency**: All StructFrame operations (`filter`, `to_dict`, `to_csv`, `validate`) work seamlessly on LazyFrames
- **`sf_collect()` method**: Escape hatch to materialize LazyFrames when needed

#### Backend-Specific Typing
- **`structframe.typing.pandas`**: Pandas-specific `Col[T]` → `pd.Series[T]` for perfect IDE autocomplete
- **`structframe.typing.polars`**: Polars-specific `Col[T]` as `class Col(pl.Expr, Generic[T])` — preserves inner type `T` while giving Polars expression method autocomplete
- **Generic fallback**: Default `structframe.typing` works for any backend
...
- **Error translation**: Pandera exceptions automatically translated to StructFrame exception types (`MissingColumnError`, `TypeMismatchError`, `ConstraintViolationError`)
...
[0.3.0]: https://github.com/yourusername/structframe/compare/v0.2.0...v0.3.0
[0.2.0]: https://github.com/yourusername/structframe/compare/v0.1.0...v0.2.0
[0.1.0]: https://github.com/yourusername/structframe/releases/tag/v0.1.0
