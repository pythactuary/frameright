...
class TestPolarsPropertyAccess:
    ...
    def test_alias_mapping(self, valid_df):
        ...
        users = UserData(valid_df)
        result = users.pf_data.select(users.tier).to_series().to_list()
    ...
    def test_setter_with_series(self, valid_df):
        ...
        result = users.pf_data.select(users.user_id).to_series().to_list()
...
class TestPolarsCoreMethods:
    def test_pf_data_returns_polars_df(self, valid_df):
        """pf_data returns the underlying Polars DataFrame."""
        users = UserData(valid_df)
        assert isinstance(users.pf_data, pl.DataFrame)

    def test_pf_filter_with_expr(self, valid_df):
        """pf_filter works with pl.Expr from property getter."""
        users = UserData(valid_df)
        active = users.pf_filter(users.is_active)
        assert len(active) == 2
        assert isinstance(active, UserData)

    def test_pf_filter_with_comparison_expr(self, valid_df):
        """pf_filter works with comparison expressions."""
        users = UserData(valid_df)
        high = users.pf_filter(users.engagement_score > 50.0)
        assert len(high) == 2

    def test_pf_to_dict(self, valid_df):
        """pf_to_dict returns a list of dicts."""
        users = UserData(valid_df)
        d = users.pf_to_dict(orient="records")
        assert isinstance(d, list)
        assert len(d) == 3

    def test_pf_to_csv(self, valid_df, tmp_path):
        """pf_to_csv writes a CSV file."""
        users = UserData(valid_df)
        path = str(tmp_path / "test.csv")
        users.pf_to_csv(path)
...
class TestCrossBackendConsistency:
    ...
    def test_to_dict_consistent(self):
        ...
        pd_dict = pd_obj.pf_to_dict(orient="records")
        pl_dict = pl_obj.pf_to_dict(orient="records")
...
class TestPolarsLazyFrame:
    ...
    def test_lazyframe_filter_with_expr(self, valid_df):
        """pf_filter works on LazyFrame with expressions."""
        lf = valid_df.lazy()
        users = UserData(lf)
        active = users.pf_filter(users.is_active)
...
# ===========================================================================
# SECTION 14: pf_collect() — LazyFrame materialisation
# ===========================================================================


class TestSfCollect:
    """Tests for the pf_collect() materialisation escape hatch."""

    def test_collect_lazyframe_returns_dataframe(self, valid_df):
        """pf_collect() materialises a LazyFrame into an eager ProteusFrame."""
        lf = valid_df.lazy()
        users = UserData(lf)
        assert isinstance(users.pf_data, pl.LazyFrame)

        collected = users.pf_collect()
        assert isinstance(collected, UserData)
        assert isinstance(collected.pf_data, pl.DataFrame)
        assert collected.pf_data.height == 3

    def test_collect_eager_returns_self(self, valid_df):
        """pf_collect() on an eager DataFrame returns self unchanged."""
        users = UserData(valid_df)
        same = users.pf_collect()
    ...
    def test_collect_preserves_data(self, valid_df):
        ...
        eager_users = lazy_users.pf_collect()
        assert eager_users.pf_data.to_dicts() == valid_df.to_dicts()

    def test_collect_then_access_series(self, valid_df):
        """After collect, get_column returns an actual pl.Series."""
        lf = valid_df.lazy()
        users = UserData(lf).pf_collect()
        series = users.pf_backend.get_column(users.pf_data, "user_id")
    ...
    def test_collect_after_filter(self, valid_df):
        ...
        collected = active.pf_collect()
        assert isinstance(collected.pf_data, pl.DataFrame)
        assert collected.pf_data.height == 2

    def test_collect_pandas_noop(self):
        """pf_collect() is a no-op for Pandas backend."""
        import pandas as pd
